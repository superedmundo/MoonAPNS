...
namespace MoonAPNS
{
  public class PushNotification
  {
    ...
    public PushNotification(bool useSandbox, X509Certificate2 certificadop12)
    {
        if (useSandbox)
        {
            _host = SandboxHost;
    ...
    public List<string> SendToAppleComunica(List<NotificationPayload> queue)
     {
        //Logger.Info("Payload queue received.");
        _notifications = queue;
        if (queue.Count < 8999)
    ...
    private void ReadResponse(IAsyncResult ar)
    {
      ...
      try
      {
        ...
        if (_apnsStream.CanRead)
        {
          ...
          _rejected.Add(_notifications[payLoadIndex].IdMensaje.ToString() + " " +_errorList[status]);
          _conected = false;
        }
      }
      catch (Exception)
    ...
    private static byte[] GeneratePayload(NotificationPayload payload)
    {
      ...
      // String length
       ...
       ////  Logger.Info("Payload generated for " + payload.DeviceToken + " : " + apnMessage);
          ...
          //http://stackoverflow.com/questions/15345162/moon-apns-message-with-accent'
